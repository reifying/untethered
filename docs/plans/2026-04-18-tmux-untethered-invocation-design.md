...
- **Cursor:** No explicit terminal marker. Emit `turn_complete` when `~/.cursor/projects/<project-hash>/agent-transcripts/<session-uuid>.jsonl` has mtime stable for ≥3 seconds AND its last JSON record has `role: "assistant"`. TUI write path (via `CliTranscriptWriter` on every `handleCheckpoint`) verified from shipped source of `cursor-agent@2026.02.13-41ac335`. See `docs/provider-cli-reference.md` §5.3 for the evidence trail, threshold rationale, edge-case handling, and version-sensitivity note. Live-session sanity-check is pending (tmux-untethered-58q).
