...
# Build and install React Native iOS app to connected iPhone
rn-deploy-device:
	@echo "Building and deploying React Native iOS app to connected iPhone..."
	@echo "Make sure your iPhone is connected via USB and unlocked"
	$(WRAP) bash -c "cd $(RN_IOS_DIR) && xcodebuild build -workspace VoiceCodeMobile.xcworkspace -scheme VoiceCodeMobile -destination 'generic/platform=iOS' -allowProvisioningUpdates -derivedDataPath build CODE_SIGN_STYLE=Automatic DEVELOPMENT_TEAM=$(DEVELOPMENT_TEAM)"
	@echo "Installing to device..."
