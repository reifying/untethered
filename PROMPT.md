Find something to improve in our new CLJS React Native code base (`./frontend/**`). We are working on replacing our Swift iOS app (`./ios/**`) with a React Native cross-platform equivalent (at `./frontend`).
...
It is your job to determine the what is important to work on next to deliver a high quality, finished platform-agnostic mobile app. We have not done much manual testing yet so any code in `./frontend/**` is still suspect and may have bugs. Much of it still does not meet our code standards. Any bug fix or refactors need to ensure we still have parity with the iOS original implementation within reason.
