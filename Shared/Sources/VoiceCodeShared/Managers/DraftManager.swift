...
public class DraftManager: ObservableObject {
    ...
    public func saveDraft(sessionID: String, text: String) {
        if text.isEmpty {
    ...
    public func getDraft(sessionID: String) -> String {
        return drafts[sessionID] ?? ""
    }

    /// Clear draft for a session (e.g., after sending prompt)
    public func clearDraft(sessionID: String) {
        drafts.removeValue(forKey: sessionID)
    }

    /// Remove draft for a deleted session (cleanup)
    public func cleanupDraft(sessionID: String) {
        drafts.removeValue(forKey: sessionID)
    }
}
