...
public class RenderTracker {
    ...
    public static func count(_ type: Any.Type) {
        let name = String(describing: type)
        counts[name, default: 0] += 1
    }

    /// Get top N most-rendered views, sorted by count descending
    public static func topRendered(limit: Int = 10) -> [(name: String, count: Int)] {
        counts
    ...
    public static func allCounts() -> [(name: String, count: Int)] {
        counts
            .sorted { $0.value > $1.value }
            .map { (name: $0.key, count: $0.value) }
    }
    ...
    public static func metadata() -> (viewCount: Int, totalRenders: Int, elapsedSeconds: TimeInterval) {
        let elapsed = Date().timeIntervalSince(startTime)
        return (
