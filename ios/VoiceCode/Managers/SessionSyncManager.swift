    ...
    func reconcileGhostPrompt(sessionId: UUID, effectivePrompt: String) {
        ...
        persistenceController.performBackgroundTask { [weak self] context in
            ...
            if let optimistic = Self.ghostBubble(targetId: targetId, sessionId: sessionId, in: context) {
                ...
                logger.info("👻 Reconciled ghost optimistic bubble for session \(sessionIdStr)")
            } else {
                ...
                guard !Self.sessionHasMessageContaining(effectivePrompt, sessionId: sessionId, in: context) else {
                    logger.info("👻 ghost_prompt P already present for session \(sessionIdStr); skipping duplicate")
                    return
                }
                let fetch = CDBackendSession.fetchBackendSession(id: sessionId)
                guard let session = try? context.fetch(fetch).first else {
                    logger.warning("👻 ghost_prompt for unknown session \(sessionIdStr); dropping effective prompt")
                    return
                }
                let messageId = UUID()
                ...
                logger.info("👻 Created fallback ghost bubble for session \(sessionIdStr)")
            }

            do {
                if context.hasChanges { try context.save() }
            } catch {
                logger.error("Failed to save ghost reconcile for \(sessionIdStr): \(error.localizedDescription)")
            }
        }
    }
    ...
    func failGhostPrompt(sessionId: UUID) {
        ...
        persistenceController.performBackgroundTask { [weak self] context in
            ...
            guard let optimistic = Self.ghostBubble(targetId: targetId, sessionId: sessionId, in: context) else {
                logger.info("👻 No optimistic ghost bubble to fail for session \(sessionIdStr)")
                return
            }
            ...
            do {
                ...
                logger.error("Failed to save ghost failure for \(sessionIdStr): \(error.localizedDescription)")
            }
        }
    }
