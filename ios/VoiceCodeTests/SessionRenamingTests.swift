//
//  SessionRenamingTests.swift
//  VoiceCodeTests
//
//  Tests for session renaming (voice-code-96)
//

import XCTest
import CoreData
@testable import VoiceCode

class SessionRenamingTests: XCTestCase {
    var persistenceController: PersistenceController!
    var context: NSManagedObjectContext!

    override func setUp() {
        super.setUp()
        persistenceController = PersistenceController(inMemory: true)
        context = persistenceController.container.viewContext
    }

    override func tearDown() {
        context = nil
        persistenceController = nil
        super.tearDown()
    }

    // MARK: - Session Renaming Tests

    func testSetLocalNameOnSession() throws {
        // Create a session
        let sessionId = UUID()
        let session = CDBackendSession(context: context)
        session.id = sessionId
        session.backendName = "Backend Name"
        session.workingDirectory = "/test"
        session.lastModified = Date()
        session.messageCount = 0
        session.preview = ""
        session.unreadCount = 0
        session.isLocallyCreated = false

        try context.save()

        // Set custom name via CDUserSession
        let userSession = CDUserSession(context: context)
        userSession.id = sessionId
        userSession.createdAt = Date()
        userSession.customName = "My Custom Name"
        userSession.isUserDeleted = false
        try context.save()

        // Verify custom name is set
        let fetchRequest = CDBackendSession.fetchBackendSession(id: sessionId)
        let updatedSession = try context.fetch(fetchRequest).first

        XCTAssertEqual(updatedSession?.displayName(context: context), "My Custom Name")
        XCTAssertEqual(updatedSession?.backendName, "Backend Name")
    }

    func testDisplayNameUsesLocalNameWhenSet() throws {
        // Create a session with custom local name
        let sessionId = UUID()
        let session = CDBackendSession(context: context)
        session.id = sessionId
        session.backendName = "Backend Name"
        session.workingDirectory = "/test"
        session.lastModified = Date()
        session.messageCount = 0
        session.preview = ""
        session.unreadCount = 0
        session.isLocallyCreated = false

        // Create user session with custom name
        let userSession = CDUserSession(context: context)
        userSession.id = sessionId
        userSession.createdAt = Date()
        userSession.customName = "Custom Name"
        userSession.isUserDeleted = false

        try context.save()

        // Display name should be custom local name
        XCTAssertEqual(session.displayName(context: context), "Custom Name")
    }

    func testDisplayNameFallsBackToBackendName() throws {
        // Create a session without custom name
        let session = CDBackendSession(context: context)
        session.id = UUID()
        session.backendName = "Backend Name"
        session.workingDirectory = "/test"
        session.lastModified = Date()
        session.messageCount = 0
        session.preview = ""
        session.unreadCount = 0
        session.isLocallyCreated = false

        try context.save()

        // Display name should fall back to backend name (no CDUserSession exists)
        XCTAssertEqual(session.displayName(context: context), "Backend Name")
    }

    func testCanRenameSessionMultipleTimes() throws {
        // Create a session
        let sessionId = UUID()
        let session = CDBackendSession(context: context)
        session.id = sessionId
        session.backendName = "Original"
        session.workingDirectory = "/test"
        session.lastModified = Date()
        session.messageCount = 0
        session.preview = ""
        session.unreadCount = 0
        session.isLocallyCreated = false

        // Create user session
        let userSession = CDUserSession(context: context)
        userSession.id = sessionId
        userSession.createdAt = Date()
        userSession.isUserDeleted = false

        try context.save()

        // First rename
        userSession.customName = "First Rename"
        try context.save()
        XCTAssertEqual(session.displayName(context: context), "First Rename")

        // Second rename
        userSession.customName = "Second Rename"
        try context.save()
        XCTAssertEqual(session.displayName(context: context), "Second Rename")

        // Third rename
        userSession.customName = "Final Name"
        try context.save()
        XCTAssertEqual(session.displayName(context: context), "Final Name")
    }

    func testClearingLocalNameRevertsToBackendName() throws {
        // Create a session with custom name
        let sessionId = UUID()
        let session = CDBackendSession(context: context)
        session.id = sessionId
        session.backendName = "Backend Name"
        session.workingDirectory = "/test"
        session.lastModified = Date()
        session.messageCount = 0
        session.preview = ""
        session.unreadCount = 0
        session.isLocallyCreated = false

        // Create user session with custom name
        let userSession = CDUserSession(context: context)
        userSession.id = sessionId
        userSession.createdAt = Date()
        userSession.customName = "Custom Name"
        userSession.isUserDeleted = false

        try context.save()
        XCTAssertEqual(session.displayName(context: context), "Custom Name")

        // Clear custom name
        userSession.customName = nil
        try context.save()

        // Should revert to backend name
        XCTAssertEqual(session.displayName(context: context), "Backend Name")
    }

    func testRenamingDoesNotAffectBackendName() throws {
        // Create a session
        let sessionId = UUID()
        let session = CDBackendSession(context: context)
        session.id = sessionId
        session.backendName = "Backend Name"
        session.workingDirectory = "/test"
        session.lastModified = Date()
        session.messageCount = 0
        session.preview = ""
        session.unreadCount = 0
        session.isLocallyCreated = false

        try context.save()

        // Rename using CDUserSession
        let userSession = CDUserSession(context: context)
        userSession.id = sessionId
        userSession.createdAt = Date()
        userSession.customName = "Custom Name"
        userSession.isUserDeleted = false
        try context.save()

        // Verify backend name is unchanged
        let fetchRequest = CDBackendSession.fetchBackendSession(id: sessionId)
        let updatedSession = try context.fetch(fetchRequest).first

        XCTAssertEqual(updatedSession?.backendName, "Backend Name")
        XCTAssertEqual(updatedSession?.displayName(context: context), "Custom Name")
    }

    func testRenamingDoesNotAffectOtherSessions() throws {
        // Create two sessions
        let sessionId1 = UUID()
        let session1 = CDBackendSession(context: context)
        session1.id = sessionId1
        session1.backendName = "Session 1"
        session1.workingDirectory = "/test1"
        session1.lastModified = Date()
        session1.messageCount = 0
        session1.preview = ""
        session1.unreadCount = 0
        session1.isLocallyCreated = false

        let sessionId2 = UUID()
        let session2 = CDBackendSession(context: context)
        session2.id = sessionId2
        session2.backendName = "Session 2"
        session2.workingDirectory = "/test2"
        session2.lastModified = Date()
        session2.messageCount = 0
        session2.preview = ""
        session2.unreadCount = 0
        session2.isLocallyCreated = false

        try context.save()

        // Rename session 1 via CDUserSession
        let userSession1 = CDUserSession(context: context)
        userSession1.id = sessionId1
        userSession1.createdAt = Date()
        userSession1.customName = "Renamed Session 1"
        userSession1.isUserDeleted = false
        try context.save()

        // Verify session 2 is unaffected
        let fetchRequest = CDBackendSession.fetchBackendSession(id: sessionId2)
        let unaffectedSession = try context.fetch(fetchRequest).first

        XCTAssertEqual(unaffectedSession?.backendName, "Session 2")
        XCTAssertEqual(unaffectedSession?.displayName(context: context), "Session 2")
    }

    func testRenamingPreservesSessionData() throws {
        // Create a session with messages and metadata
        let sessionId = UUID()
        let session = CDBackendSession(context: context)
        session.id = sessionId
        session.backendName = "Original Name"
        session.workingDirectory = "/test"
        session.lastModified = Date()
        session.messageCount = 3
        session.preview = "Last message text"
        session.unreadCount = 1
        session.isLocallyCreated = false

        // Add messages
        for i in 1...3 {
            let message = CDMessage(context: context)
            message.id = UUID()
            message.sessionId = sessionId
            message.role = i % 2 == 0 ? "assistant" : "user"
            message.text = "Message \(i)"
            message.timestamp = Date()
            message.messageStatus = .confirmed
            message.session = session
        }

        try context.save()

        // Rename session via CDUserSession
        let userSession = CDUserSession(context: context)
        userSession.id = sessionId
        userSession.createdAt = Date()
        userSession.customName = "New Name"
        userSession.isUserDeleted = false
        try context.save()

        // Verify all session data is preserved
        let fetchRequest = CDBackendSession.fetchBackendSession(id: sessionId)
        let renamedSession = try context.fetch(fetchRequest).first

        XCTAssertNotNil(renamedSession)
        XCTAssertEqual(renamedSession?.displayName(context: context), "New Name")
        XCTAssertEqual(renamedSession?.backendName, "Original Name")
        XCTAssertEqual(renamedSession?.messageCount, 3)
        XCTAssertEqual(renamedSession?.preview, "Last message text")
        XCTAssertEqual(renamedSession?.unreadCount, 1)

        // Verify messages are preserved
        let messages = renamedSession?.messages?.allObjects as? [CDMessage]
        XCTAssertEqual(messages?.count, 3)
    }

    func testLocalNamePersistsAcrossFetches() throws {
        // Create and rename a session
        let sessionId = UUID()
        let session = CDBackendSession(context: context)
        session.id = sessionId
        session.backendName = "Backend Name"
        session.workingDirectory = "/test"
        session.lastModified = Date()
        session.messageCount = 0
        session.preview = ""
        session.unreadCount = 0
        session.isLocallyCreated = false

        // Create user session with custom name
        let userSession = CDUserSession(context: context)
        userSession.id = sessionId
        userSession.createdAt = Date()
        userSession.customName = "Custom Name"
        userSession.isUserDeleted = false

        try context.save()

        // Clear context to force re-fetch from store
        context.refreshAllObjects()

        // Fetch session again
        let fetchRequest = CDBackendSession.fetchBackendSession(id: sessionId)
        let refetchedSession = try context.fetch(fetchRequest).first

        // Verify custom name persisted
        XCTAssertEqual(refetchedSession?.displayName(context: context), "Custom Name")
    }

    func testEmptyLocalNameTreatedAsNil() throws {
        // Create a session
        let sessionId = UUID()
        let session = CDBackendSession(context: context)
        session.id = sessionId
        session.backendName = "Backend Name"
        session.workingDirectory = "/test"
        session.lastModified = Date()
        session.messageCount = 0
        session.preview = ""
        session.unreadCount = 0
        session.isLocallyCreated = false

        // Create user session with empty custom name
        let userSession = CDUserSession(context: context)
        userSession.id = sessionId
        userSession.createdAt = Date()
        userSession.customName = ""  // Empty string
        userSession.isUserDeleted = false

        try context.save()

        // Empty string should fall back to backend name
        // displayName(context:) only returns customName if it's non-nil
        // Empty string is technically non-nil, so this test documents that behavior
        let displayName = session.displayName(context: context)
        // If empty string is treated as valid, displayName will be ""
        // If it falls back to backend name, displayName will be "Backend Name"
        XCTAssertTrue(displayName == "" || displayName == "Backend Name")
    }
}
